...
import AttendanceTrendsChart from './components/singlePerson_analytics/AttendanceTrendsChart';
import AttendancePatternsChart from './components/singlePerson_analytics/AttendancePatternsChart';
import AttendanceAnalytics from './components/singlePerson_analytics/AttendanceAnalytics';
...
export default function AttendanceSingleStudent() {
    ...
    return (
        <div className="w-full  h-full flex flex-col space-y-6 animate-in fade-in bg-background p-2 md:p-2 relative">
            ...
            {isLoading ? (
                ...
                <div className="flex-1 min-h-0 overflow-y-auto gap-5 lg:gap-6">
                    ...
                    {/* <div className="w-full bg-surface border border-border rounded-2xl shadow-sm flex flex-col shrink-0" style={{ minHeight: '400px' }}> */}
                    {/* <div className="w-full bg-surface border border-border rounded-2xl shadow-sm  shrink-0 h-[450px] lg:h-[500px]"> */}
                    ...
                    {/* BOTTOM: Advanced Analytics Grid */}
                    <div className="flex flex-col gap-6 shrink-0 mt-20">

                        {/* Row 1: Donut (1/3) & Line Chart (2/3) */}
                        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                            ...
                            <div className="xl:col-span-2 h-[420px]">
                                {/* 🌟 Pass the academicYear from filters so it updates dynamically */}
                        ...
                        {/* Row 2: Pattern Bar Chart (Full Width) */}
...
//                             className={`aspect-square rounded-xl flex flex-col items-center justify-center border transition-all relative group ${!recordForDay ? 'bg-background border-border/50 opacity-40 cursor-not-allowed'
...
//                                 }`}
//                         >
//                             <span className={`text-base sm:text-xl font-black ${!recordForDay ? 'text-muted'
...
//                                     <div className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full ${isPresent ? 'bg-success' : isAbsent ? 'bg-danger' : isLate ? 'bg-warning' : 'bg-primary'
//                                         }`}></div>
//                                 </div>
//                             )}
//                         </button>
    ...
    return (
        <div className="max-w-full mx-auto w-full">
            ...
            <div className="grid grid-cols-7 gap-2 mb-3">
                {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
                    <div key={day} className="text-center text-[10px] sm:text-xs font-bold text-muted uppercase tracking-wider py-1">
                        {day}
                    </div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-7 gap-2 sm:gap-3">
                {/* Blank placeholders for the start of the month */}
                {blankDays.map(blank => (
                    <div
                        key={`blank-${blank}`}
                        // 🌟 Fixed heights instead of aspect-square, with a subtle dashed border
                        className="h-16 sm:h-20 md:h-24 rounded-xl border-2 border-dashed border-border/40 bg-surface/30"
                    ></div>
                ))}
                ...
                {monthDays.map(day => {
                    ...
                    return (
                        <button
                            ...
                            onClick={() => onDayClick(recordForDay)}
                            title={recordForDay ? `Click to view details for ${day}` : 'No data recorded'}
                            // 🌟 Replaced aspect-square with fixed heights and improved border definitions
                            className={`h-16 sm:h-20 md:h-24 rounded-xl flex flex-col items-center justify-center transition-all relative group 
                                ${!recordForDay
                                    ? 'bg-surface border border-border opacity-50 cursor-not-allowed'
                                    : isPresent
                                        ? 'bg-success/10 border-2 border-success/30 hover:border-success/60 hover:bg-success/20 cursor-pointer shadow-sm hover:shadow-md'
                                        : isAbsent
                                            ? 'bg-danger/10 border-2 border-danger/30 hover:border-danger/60 hover:bg-danger/20 cursor-pointer shadow-sm hover:shadow-md'
                                            : isLate
                                                ? 'bg-warning/10 border-2 border-warning/30 hover:border-warning/60 hover:bg-warning/20 cursor-pointer shadow-sm hover:shadow-md'
                                                : isHalfDay
                                                    ? 'bg-primary/10 border-2 border-primary/30 hover:border-primary/60 hover:bg-primary/20 cursor-pointer shadow-sm hover:shadow-md'
                                                    : 'bg-surface border-2 border-border hover:border-primary/50 cursor-pointer'
                                }`}
                        >
