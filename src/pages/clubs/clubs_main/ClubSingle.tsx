...
// import { useGetClubById } from '../../../api_services/clubs_api/clubApi';
// import {
...
//     // --- Queries ---
//     const { data: clubData, isLoading: isClubLoading } = useGetClubById(id);
//     const {
//         data: videosData,
//         fetchNextPage,
//         hasNextPage,
...
//     } = useGetAllClubVideosInfinite({ clubId: id! });
...
//     const deletePdfMutation = useDeleteClubVideoFile();
...
//     // --- Handlers: Upload New Video (Sequential Fix for Multer) ---
//     const handleUploadSubmit = async () => {
//         if (!videoFile || !id) return;

//         try {
//             // 1. Upload Video & Details
//             const videoFormData = new FormData();
//             videoFormData.append('schoolId', schoolId!);
//             videoFormData.append('clubId', id);
...
//             videoFormData.append('video', videoFile); // Backend upload.single('video')

//             const response = await uploadVideoMutation.mutateAsync(videoFormData);
//             const newVideoId = response?.data?._id;

//             // 2. Upload PDFs (If any, to the newly created video ID)
//             if (pdfFiles.length > 0 && newVideoId) {
//                 const pdfFormData = new FormData();
//                 pdfFiles.forEach(pdf => pdfFormData.append('files', pdf)); // Backend upload.array('files')
...
//     // --- Handlers: Edit Existing Video ---
//     const openEditModal = (video: any) => {
//         setEditingVideo(video);
//         setEditData({
//             title: video.title || '',
//             topic: video.topic || '',
//             level: video.level || 'general'
//         });
...
//             // 2. Replace Video File (If selected)
//             if (editVideoFile) {
...
//             setEditingVideo(null); // Close modal on success
//         } catch (error) {
...
//     if (isClubLoading) {
//         return <div className="flex h-full items-center justify-center"><i className="fas fa-circle-notch fa-spin text-primary text-4xl"></i></div>;
//     }
...
//         updateVideoFileMutation.isPending;
...
//                     <button onClick={() => navigate(-1)} className="w-8 h-8 flex items-center justify-center rounded-full border border-gray-200 text-gray-500 hover:bg-gray-50 transition-colors">
...
//                             {clubData?.studentId?.length || 0} Members
//                         </p>
//                     </div>
//                 </div>
//                 {isAdmin && (
//                     <Button variant="primary" leftIcon="fas fa-cloud-upload-alt" onClick={() => setIsUploadModalOpen(true)}>
//                         Upload
//                     </Button>
//                 )}
//             </header>
...
//                                         {/* HTML5 Video Player */}
//                                         <div className="relative w-full bg-black aspect-video border-b border-gray-100">
//                                             {vid.video?.url ? (
//                                                 <video
//                                                     src={vid.video.url}
//                                                     controls
...
//                                                 />
//                                             ) : (
//                                                 <div className="w-full h-full flex items-center justify-center text-gray-500 text-sm">Video Processing...</div>
//                                             )}
//                                         </div>
...
//                                                         {vid.pdfs.map((pdf: any, idx: number) => (
...
//                                                                     <span className="truncate text-[11px] font-medium text-gray-700" title={pdf.originalName}>
//                                                                         {pdf.originalName}
//                                                                     </span>
//                                                                 </div>

//                                                                 {/* Permanent Actions (View & Delete) */}
...
//                                                                             className="w-6 h-6 flex items-center justify-center text-rose-500 hover:bg-rose-100 rounded transition-colors"
//                                                                             title="Delete File"
//                                                                         >
...
//                                         {/* Admin Action Bar */}
//                                         {isAdmin && (
//                                             <div className="bg-gray-50 border-t border-gray-100 flex items-center justify-between p-1.5 px-3">
//                                                 <button
//                                                     onClick={() => openEditModal(vid)}
//                                                     className="flex items-center gap-1.5 text-[10px] font-bold text-gray-500 hover:text-primary transition-colors py-1.5"
//                                                 >
...
//                                                 <button
//                                                     onClick={() => {
//                                                         if (window.confirm("Permanently delete this video?")) deleteVideoMutation.mutateAsync(vid._id);
//                                                     }}
//                                                     className="flex items-center gap-1.5 text-[10px] font-bold text-rose-500 hover:text-rose-700 transition-colors py-1.5"
...
//                         <input type="file" accept="video/mp4,video/mkv" className="text-sm bg-white p-1 rounded border border-blue-200" onChange={(e) => setVideoFile(e.target.files?.[0] || null)} />
//                     </div>
...
//                         <input type="file" accept="application/pdf" multiple className="text-sm bg-white p-1 rounded border border-gray-300" onChange={(e) => setPdfFiles(Array.from(e.target.files || []))} />
//                         <span className="text-[10px] text-gray-500 mt-1">You can select multiple files at once.</span>
//                     </div>

//                     <div className="pt-4 mt-auto">
//                         <Button variant="primary" className="w-full py-3.5" onClick={handleUploadSubmit} isLoading={isAnyMutationPending} disabled={!videoFile || !uploadData.title}>
...
//                         <input type="file" accept="video/mp4,video/mkv" className="border border-gray-300 p-2 rounded-lg text-sm" onChange={(e) => setEditVideoFile(e.target.files?.[0] || null)} />
//                         <span className="text-[10px] text-gray-500">Leave blank to keep the current video.</span>
//                     </div>

//                     <div className="flex flex-col gap-1.5 pt-4 border-t border-gray-200">
//                         <Label>Append Additional PDFs (Optional)</Label>
//                         <input type="file" accept="application/pdf" multiple className="border border-gray-300 p-2 rounded-lg text-sm" onChange={(e) => setEditPdfFiles(Array.from(e.target.files || []))} />
//                     </div>

//                     <div className="pt-6 mt-auto">
//                         <Button variant="primary" className="w-full py-3.5" onClick={handleEditSubmit} isLoading={isAnyMutationPending} disabled={!editData.title}>
//                             Save Changes
//                         </Button>
//                     </div>
//                 </div>
            ...
            <header className="shrink-0 px-6 py-5 border-b border-gray-200 flex items-center justify-between bg-white z-10 shadow-sm">
                ...
                {isAdmin && (
                    <div className="flex items-center gap-3">
                        <Button variant="outline" leftIcon="fas fa-users-cog" onClick={() => setIsStudentModalOpen(true)} className="cursor-pointer">
                            Manage Members
                        </Button>
                        <Button variant="primary" leftIcon="fas fa-cloud-upload-alt" onClick={() => setIsUploadModalOpen(true)} className="cursor-pointer">
                            Upload Lesson
                        </Button>
                    </div>
                )}
                    ...
                    <div className="bg-surface border border-border p-5 rounded-2xl shadow-sm space-y-4">
                        ...
                        {canFetchStudents && allStudents.length > 0 && (
                            <div className="pt-3 border-t border-border mt-2">
                                <Button 
                                    variant="outline" 
                                    className="w-full text-xs font-bold cursor-pointer border-primary/30 text-primary hover:bg-primary hover:text-white"
