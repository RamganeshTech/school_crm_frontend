...
export default function FeeCollectionSingle() {
    ...
    // --- API Hooks ---
    const { data: record, isLoading: isRecordLoading, refetch } = useGetStudentRecordByIdV1(schoolId!, studentId, academicYear);
    ...
    return (
        ...
        <div className="w-full h-full flex flex-col bg-background overflow-hidden animate-in fade-in">
            ...
            {/* --- 1. SLIM HEADER --- */}
            <header className="shrink-0 px-4 py-3 border-b border-border flex items-center gap-3 bg-surface z-10 shadow-sm">
            ...
            <div className="flex-1 flex flex-col p-3 lg:p-4 gap-4 overflow-hidden">
                ...
                <div className="shrink-0 px-5 py-3 bg-surface border border-border rounded-xl shadow-sm flex flex-col sm:flex-row justify-between sm:items-center gap-3 z-10">
                                            ...
                                            <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                                                {[500, 200, 100, 50, 20, 10].map((note) => (
                                                    <div key={note} className="flex flex-col gap-1 bg-surface border border-border px-2 py-1.5 rounded-lg text-center shadow-sm">
                                                        ...
                                                        <input type="number" min="0" placeholder="0" className="w-full bg-background border border-border/60 rounded p-1 text-xs text-foreground text-center font-bold outline-none focus:border-primary/50" value={denominations[`notes${note}` as keyof typeof denominations] || ''} onChange={(e) => setDenominations({ ...denominations, [`notes${note}`]: Number(e.target.value) || 0 })} />
                                                    </div>
                                                ))}
                                            </div>
